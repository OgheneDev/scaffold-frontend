  ...
  return (
    <div className="max-w-xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-fg-muted">Manage your profile, security, and account.</p>
      </div>
      ...
      <Card>
        <form onSubmit={profileForm.handleSubmit(onProfileSubmit)}>
          <CardHeader>
            ...
            <CardDescription>Your name is shown across the dashboard.</CardDescription>
          </CardHeader>
          ...
          <CardFooter>
            <Button type="submit" size="sm" disabled={profileForm.formState.isSubmitting}>
              {profileForm.formState.isSubmitting ? <Loader2 className="size-4 animate-spin" /> : null}
              Save changes
            </Button>
          </CardFooter>
        </form>
      ...
      <Card className="border-danger/30">
        <CardHeader>
          ...
          <CardDescription>Permanently delete your account and every site in it.</CardDescription>
        </CardHeader>
        <CardFooter>
            ...
            <AlertDialogContent>
              <form onSubmit={deleteForm.handleSubmit(onDeleteSubmit)}>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This deletes your account and all sites permanently. Confirm with your password.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="space-y-1.5">
                  ...
                  {deleteForm.formState.errors.password ? (
                    <p className="text-xs text-danger">{deleteForm.formState.errors.password.message}</p>
                  ) : null}
                </div>
                <AlertDialogFooter>
                  <AlertDialogCancel type="button">Cancel</AlertDialogCancel>
                  <Button type="submit" variant="destructive" disabled={deleteForm.formState.isSubmitting}>
                    {deleteForm.formState.isSubmitting ? <Loader2 className="size-4 animate-spin" /> : null}
                    Delete account
                  </Button>
                </AlertDialogFooter>
              </form>
